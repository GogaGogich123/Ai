...
from text_encoder import TextConditionedTransformer
from blocks import BLOCKS_ARRAY

class TextLatentDataset(torch.utils.data.IterableDataset):
    def __init__(self, base_dataset, vqvae, device, mask_ratio=0.15):
        ...
        self.category_templates = {
            'house': ['a cozy house', 'a modern house', 'a small cottage', 'a family home', 'a residential building'],
            'castle': ['a medieval castle', 'a fortress', 'a stone castle', 'a royal castle', 'a defensive castle'],
            'modern': ['a modern building', 'a contemporary structure', 'a sleek design', 'a minimalist building'],
            'fantasy': ['a fantasy building', 'a magical structure', 'an enchanted place', 'a mystical building'],
            'medieval': ['a medieval building', 'a stone structure', 'an old building', 'a historic structure'],
            'farm': ['a farm building', 'a barn', 'a farmhouse', 'a rural structure'],
            'shop': ['a shop', 'a store', 'a market stall', 'a trading post'],
            'tower': ['a tower', 'a tall structure', 'a watchtower', 'a spire'],
        }
    
    def generate_prompt(self, category: str) -> str:
        if category in self.category_templates:
            return random.choice(self.category_templates[category])
        return f"a {category} building"
    ...
    def __iter__(self):
        ...
        with torch.no_grad():
            for sample in self.base_dataset:
                ...
                prompt = self.generate_prompt(sample['category'])
...
def collate_fn(batch):
    return {
        ...
        'build_ids': [item['build_id'] for item in batch],
...
def train_epoch(model, dataloader, optimizer, device, epoch):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        logits = model(input_indices, prompts)
...
def main(args):
    ...
    wandb.init(
        project="minecraft-text-transformer",
        config={
    ...
    latent_dataset = TextLatentDataset(
    ...
    print("Creating text-conditioned transformer...")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.save_every == 0:
            checkpoint_path = checkpoint_dir / f"text_transformer_epoch_{epoch+1}.pt"
            torch.save({
                'epoch': epoch,
    ...
    final_path = checkpoint_dir / "text_transformer_final.pt"
    torch.save({
        'model_state_dict': model.state_dict(),
        'config': {
            'num_embeddings': vqvae.vq.num_embeddings,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpoints_text')
